...
document.addEventListener('DOMContentLoaded', () => {
  mediumZoom('.zoomable', {
    // Sin margen para que llene todo
    margin: 0,
    // Fondo semitransparente
    background: 'rgba(0,0,0,0.8)',
    // Que no haga scrollOffset (no añade espacio arriba/abajo)
    scrollOffset: 0,
    // Lo inyecta en body para ocupar toda la pantalla
...
createApp({
  ...
  methods: {
    toggleZoom() {
      ...
      document.body.style.overflow = this.zoomActivo ? 'hidden' : '';
    },
    onScroll() {
    ...
    abrirModal(producto) {
      this.imagenIndex = 0; this.productoModal = producto; this.selectedColor = producto.colores[0].name; this.selectedSize = null; setLanguage(this.lang);
  ...
  mounted() {
    ...
    document.addEventListener("language-changed", e => {
      this.lang = e.detail;
    }); window.addEventListener('scroll', this.onScroll); setInterval(() => { this.imagenActual = (this.imagenActual + 1) % this.imagenesCarrusel.length; }, 5000);
  },
